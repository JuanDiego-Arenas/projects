...
public class ProductsDatabase {
    ...
    /**
     * Calcual cual de los productos tiene el mayor precio
     * @return Nombre del producto con el mayor precio
     */
    public String mayor() {
        double mayor = 0;
        String nombreMayor = "";
        for(Product producto : listaProductos.values()) {
            if(producto.getPrice() > mayor) {
                mayor = producto.getPrice();
                nombreMayor = producto.getName();
            }
        }
        return nombreMayor;
    }
    ...
    /**
     * Calcula cual de los productos tiene el menor precio
     * @return Nombre del producto con el menor precio
     */
    public String menor() {
        double menor = 1000000;
        String nombreMenor = "";
        for(Product producto : listaProductos.values()) {
            if(producto.getPrice() < menor) {
                menor = producto.getPrice();
                nombreMenor = producto.getName();
            }
        }
        return nombreMenor;
    }
    ...
    /**
     * Calcula el promedio de los precios de los productos
     * @return Promedio de precios
     */
    public double promedio() {
        double mean = 0;
        for(Product producto : listaProductos.values()) {
            mean += producto.getPrice();
        }
        return mean/listaProductos.size();
    }
